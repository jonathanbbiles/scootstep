...
**The one thing that leaves your phone.** Every dance, diagram and count tick is
built into the app and works with no signal. The exception is the song list on a
dance page: if you tap **Preview**, the app sends that song's title and artist to
Apple's public iTunes Search API (`itunes.apple.com`) in order to play Apple's
own 30-second preview and to link to the song on Apple Music or Spotify. Nothing
about you or your device is included in that request, and no identifier is
attached to it. Apple's handling of that request is governed by Apple's privacy
policy. If you never tap Preview or a song link, ScootSteps makes no network
requests at all.

**Music.** ScootSteps does not play copyrighted recordings. The audio you hear
during practice is a count track synthesized by the app. Song titles are listed
as references so you can practise to the real record in your own music app.
